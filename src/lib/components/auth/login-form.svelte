<script lang="ts">
	...
	let { class: className, redirectTo = '', ...restProps }: HTMLAttributes<HTMLDivElement> = $props();

	const id = $props.id();
	const googleHref = $derived(
		redirectTo ? `/auth/login/google?redirectTo=${encodeURIComponent(redirectTo)}` : '/auth/login/google'
	);
</script>
...
<div class={cn('flex flex-col gap-6', className)} {...restProps}>
	<Card.Root>
		<Card.Header class="text-center">
			<Card.Title class="text-xl">Ravi de vous revoir</Card.Title>
			...
			<form>
				<FieldGroup>
					<Field>
						...
						<Button variant="outline" type="button" href={googleHref}>
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
								<path
