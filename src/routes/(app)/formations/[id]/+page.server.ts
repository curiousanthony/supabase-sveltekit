...
export const load = (async ({params}) => {
    ...
    const statutBadgeClass = formation.statut === 'En attente' ? '[&_svg]:text-neutral-400' : formation.statut === 'En cours' ? '[&_svg]:text-yellow-400' : formation.statut === 'Terminée' ? '[&_svg]:text-green-400' : 'bg-muted text-muted-foreground';
