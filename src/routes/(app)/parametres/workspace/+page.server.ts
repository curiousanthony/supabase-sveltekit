...
export const load = (async ({ locals, url }) => {
	const { userId, workspaceId } = await requireWorkspace({ ...locals, url } as Parameters<typeof requireWorkspace>[0]);
	const role = await getUserRoleInWorkspace(userId, workspaceId);

	// All workspace members can view; only owner/admin can manage (invite, change roles, remove)
	...
	const membersWithRoleLabel = members.map((m) => ({
		...m,
		roleLabel: getRoleLabel(m.role as import('$lib/server/workspace').WorkspaceRole)
	}));
	...
	return {
		...
		canManage,
		header: {
			pageName: 'Paramètres de l\'espace',
			actions: []
		}
	};
